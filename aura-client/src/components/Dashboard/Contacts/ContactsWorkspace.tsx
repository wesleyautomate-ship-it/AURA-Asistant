...
const DEFAULT_CONTACTS: Contact[] = [
  { id: '1', name: 'Alex Johnson', status: 'Active', lastActivity: '2h ago' },
  { id: '2', name: 'Briana Chen', status: 'New', lastActivity: 'Today' },
  { id: '3', name: 'Carlos Ramirez', status: 'Warm', lastActivity: '1d ago' },
  { id: '4', name: 'Danielle Brooks', status: 'Active', lastActivity: '3h ago' },
  { id: '5', name: 'Ethan Patel', status: 'Cold', lastActivity: '5d ago' },
  { id: '6', name: 'Fatima Khan', status: 'Warm', lastActivity: 'Yesterday' },
  { id: '7', name: 'Grace Lee', status: 'Active', lastActivity: '30m ago' },
  { id: '8', name: 'Henry Nguyen', status: 'New', lastActivity: 'Today' },
  { id: '9', name: 'Isabella Rossi', status: 'Cold', lastActivity: '1w ago' },
  { id: '10', name: 'Jack Wilson', status: 'Warm', lastActivity: '2d ago' },
];
...
export default function ContactsWorkspace({ contacts = DEFAULT_CONTACTS, onAddContact }: ContactsWorkspaceProps) {
  ...
  const beginLongPressSelect = (id: string) => {
  ...
  return (
    <section className="w-full">
      {/* Container: mobile-first, ensures header + filters visible, list scrolls */}
        ...
        <div className="px-4 pt-4 pb-3 sm:px-6 sm:pt-5 sm:pb-4 border-b border-gray-100">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-600 text-white text-xs sm:text-sm font-medium hover:bg-blue-700 active:scale-95 transition"
          ...
          {/** Filters moved to sticky bar in scroll area */}
        </div>

        {/* Scrollable area with sticky chips + search */}
        <div className="flex flex-col max-h-[60vh] sm:max-h-[65vh]">
          <div className="overflow-y-auto overscroll-contain">
            ...
            <div className="sticky top-0 z-10 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/70 border-b border-gray-100">
              <div className="px-4 pt-2 pb-2 sm:px-6">
                <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide -mx-1 px-1">
                  <div className="flex items-center text-gray-500 mr-1 shrink-0">
                    <Filter className="w-4 h-4" />
                  </div>
                  {FILTERS.map(f => (
                    <button
                      key={f}
                      onClick={() => { setActiveFilter(f); if (f !== 'Segments') setSegment(null); }}
                ...
                <div className="mt-2">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-9 pr-3 py-2 text-sm sm:text-base rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            ...
            <ul className="divide-y divide-gray-100">
                ...
                <motion.li
                  ...
                  onClick={() => {
                    if (selectionMode) {
                      ...
                      navigate(`/contacts/${c.id}` as const, {
                        state: {
                          contact: {
                            id: c.id,
                            name: c.name,
                            temperature: c.status,
                            lastActivityAt: (c as any).lastActivityIso ?? c.lastActivity,
                          },
                        },
                      });
                    }
              ...
              {filtered.length === 0 && (
                <li className="px-4 py-6 text-center text-sm text-gray-500">No contacts match your search/filter.</li>
              )}
            </ul>
            ...
            {selectedIds.size > 0 && (
              <div className="sticky bottom-0 z-10">
                <div className="m-3 rounded-xl border border-gray-200 bg-white shadow-md px-3 py-2 sm:px-4 sm:py-2 flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-gray-300 text-gray-700 text-xs sm:text-sm hover:bg-gray-50"
                      onClick={() => { /* TODO: add tag action */ }}
                    >
                      <Tag className="w-4 h-4" />
                      <span className="hidden xs:inline">Add Tag</span>
                    </button>
                    <button
                      className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-amber-600 text-white text-xs sm:text-sm hover:bg-amber-700"
    ...
    >
      {selectionMode && (
        <button
          ...
          className={`shrink-0 inline-flex items-center justify-center w-5 h-5 rounded border ${selected ? 'bg-blue-600 border-blue-600 text-white' : 'bg-white border-gray-300 text-transparent'} focus:outline-none focus:ring-2 focus:ring-blue-500`}
        >
      ...
      {!selectionMode && (
        <button
          className="ml-auto hidden sm:inline-flex items-center gap-1.5 px-2 py-1 rounded-md border border-gray-300 text-gray-700 text-xs hover:bg-gray-50"
          onClick={(e) => { e.stopPropagation(); onLongPressSelect?.(); }}
        >
          Select
        </button>
      )}
