...
async def add_team_performance(
    ...
    try:
        ...
        logger.info(
            f"Added performance metric for agent {request.agent_id}: {request.metric_name}"
        )
        return performance
...
async def get_team_analytics(
    ...
    try:
        ...
        service = BrokerageManagementService(db)
        analytics = await service.get_team_performance_summary(
            brokerage_id, period_days
        )
...
async def get_team_dashboard(
    ...
    try:
        ...
        active_agents = len(
            [m for m in team_members if m.role in ["agent", "brokerage_owner"]]
        )
