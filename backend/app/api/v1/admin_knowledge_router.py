...
async def upload_to_knowledge_base(
    ...
    try:
        ...
        # Log the upload
        logger.info(
            f"Knowledge base upload successful: {file.filename} by user {current_user.id}"
        )
...
async def get_knowledge_base_stats(current_user: User = Depends(require_admin)):
    """
...
async def delete_structured_data(
    document_type: str, record_id: int, current_user: User = Depends(require_admin)
):
    """
...
@router.get("/schemas")
async def get_document_schemas(current_user: User = Depends(get_current_user)):
    """
