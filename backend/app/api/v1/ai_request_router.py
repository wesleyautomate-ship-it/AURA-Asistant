...
class AIRequestCreate(BaseModel):
    team: str = Field(
        ...,
        description="AI team (marketing, analytics, social, strategy, packages, transactions)",
    )
    content: str = Field(..., description="Request content (text or transcript)")
    template_id: Optional[str] = Field(None, description="Template ID to use")
    brand_context: Optional[Dict[str, Any]] = Field(
        None, description="Brand context for personalization"
    )
...
async def create_ai_request(
    ...
    try:
        ...
        title = (
            request_data.content[:100] + "..."
            if len(request_data.content) > 100
            else request_data.content
        )
...
async def get_requests(
    ...
    try:
        ...
        requests = (
            query.order_by(AIRequestNew.created_at.desc())
            .offset(offset)
            .limit(limit)
            .all()
        )
...
async def get_request(
    ...
    try:
        ...
        ai_request = (
            db.query(AIRequestNew)
            .filter(
                AIRequestNew.id == request_uuid, AIRequestNew.user_id == current_user.id
            )
            .first()
        )
        ...
        # Get steps
        steps = (
            db.query(AIRequestStep)
            .filter(AIRequestStep.request_id == request_uuid)
            .order_by(AIRequestStep.created_at)
            .all()
        )

        # Get deliverables
        deliverables = (
            db.query(Deliverable).filter(Deliverable.request_id == request_uuid).all()
        )
...
async def get_templates(team: Optional[str] = None, db: Session = Depends(get_db)):
...
async def serve_brand_asset(
    brokerage_id: str, filename: str, current_user: User = Depends(get_current_user)
):
    """Serve brand assets"""
    try:
...
async def start_ai_processing(request_id: uuid.UUID, db: Session):
    ...
    try:
        ...
        ai_request = (
            db.query(AIRequestNew).filter(AIRequestNew.id == request_id).first()
        )
        if ai_request:
