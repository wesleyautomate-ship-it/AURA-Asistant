...
async def detect_property(
    request: PropertyDetectionRequest, current_user: User = Depends(get_current_user)
):
    """
    ...
    try:
        logger.info(
            f"Property detection request from user {current_user.id}: {request.message}"
        )
        ...
        if not property_info:
            raise HTTPException(
                status_code=400, detail="No property information detected"
            )
            ...
            response_data.update(
                {
                    ...
                    or db_property.get("bedrooms"),
                    "bathrooms": response_data["bathrooms"]
                    or db_property.get("bathrooms"),
                    "size_sqft": response_data["size_sqft"]
                    or db_property.get("square_feet"),
        ...
        raise HTTPException(
            status_code=500, detail=f"Property detection failed: {str(e)}"
        )


@router.post("/search", response_model=List[Dict[str, Any]])
async def search_properties(
    request: PropertySearchRequest, current_user: User = Depends(get_current_user)
):
    """
    ...
    try:
        logger.info(
            f"Property search request from user {current_user.id}: {request.dict()}"
        )
...
async def get_building_data(
    ...
    try:
        logger.info(
            f"Building data request from user {current_user.id}: {building_name} in {community}"
        )

        # Get building-specific transactions
        transactions = property_detector.get_building_specific_transactions(
            building_name, community
        )

        if not transactions:
            raise HTTPException(
                status_code=404, detail="No data found for this building"
            )
        ...
        property_types = list(
            set(
                [t.get("property_type") for t in transactions if t.get("property_type")]
            )
        )
...
async def get_community_market_data(
    ...
    try:
        logger.info(
            f"Community market data request from user {current_user.id}: {community}"
        )
        ...
        if not market_data:
            raise HTTPException(
                status_code=404, detail="No market data found for this community"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get community market data: {str(e)}"
        )
...
async def process_document(
    ...
    try:
        logger.info(
            f"Document processing request from user {current_user.id}: {file.filename}"
        )
...
async def get_document_processing_result(
    ...
    try:
        logger.info(
            f"Document result request from user {current_user.id}: {document_id}"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get document result: {str(e)}"
        )
...
async def health_check():
    ...
    """
    return {"status": "healthy", "service": "property-detection", "version": "1.0.0"}
