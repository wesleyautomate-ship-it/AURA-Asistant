...
from app.domain.listings.enhanced_real_estate_models import (
    EnhancedProperty, EnhancedClient, Transaction
)
...
async def get_command_center_data(
    ...
    try:
        # Basic counts
        total_properties = db.query(EnhancedProperty).filter(
            EnhancedProperty.is_deleted.is_(False)
        ).count()
        
        available_properties = db.query(EnhancedProperty).filter(
            EnhancedProperty.is_deleted.is_(False),
            EnhancedProperty.listing_status == "available"
        ).count()
        
        pending_properties = db.query(EnhancedProperty).filter(
            EnhancedProperty.is_deleted.is_(False),
            EnhancedProperty.listing_status == "pending"
        ).count()
        
        total_clients = db.query(EnhancedClient).count()
        active_clients = db.query(EnhancedClient).filter(
            EnhancedClient.client_status == "active"
        ).count()
        
        # Recent properties (last 5)
        recent_properties = db.query(EnhancedProperty).filter(
            EnhancedProperty.is_deleted.is_(False)
        ).order_by(desc(EnhancedProperty.id)).limit(5).all()
        
        # Recent clients (last 5)
        recent_clients = db.query(EnhancedClient).order_by(
            desc(EnhancedClient.id)
        ).limit(5).all()
        ...
        client_types = db.query(
            EnhancedClient.client_type,
            func.count(EnhancedClient.id).label('count')
        ).group_by(EnhancedClient.client_type).all()
        ...
        return {
            ...
            "metrics": {
                "properties": {
                    ...
                    "sold": total_properties - available_properties - pending_properties
                },
                "clients": {
                    "total": total_clients,
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to fetch command center data: {str(e)}"
        )
...
async def get_quick_stats(
    ...
    try:
        stats = {
            "properties_count": db.query(EnhancedProperty).filter(
                EnhancedProperty.is_deleted.is_(False)
            ).count(),
