...
async def list_packages(
    ...
    try:
        ...
        return PackageListResponse(packages=packages, total_count=len(packages))
...
async def execute_package(
    ...
    try:
        ...
        logger.info(
            f"Package {package.name} started by user {current_user.id} with execution {execution_id}"
        )
...
async def create_custom_package(
    ...
    try:
        ...
        logger.info(
            f"Custom package '{request.name}' created by user {current_user.id}"
        )
    ...
    try:
        ...
        return {
            "task_statistics": {
                ...
                "success_rate": (task_stats.completed_tasks or 0)
                / max(task_stats.total_tasks or 1, 1)
                * 100,
                "average_duration_minutes": round(
                    task_stats.avg_duration_minutes or 0, 2
                ),
            },
