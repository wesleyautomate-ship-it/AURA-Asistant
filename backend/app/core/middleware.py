...
def get_current_user(
    ...
    Args:
        credentials: HTTP authorization credentials (optional in dev mode)
        db: Database session
    ...
    if not credentials:
        logger.warning("[AUTH] No credentials provided and not in development mode")
        raise HTTPException(
            status_code=401,
...
def require_permissions(required_permissions: List[str]):
    ...
    def permission_checker(
        ...
        user_roles = (
            db.query(Role).join(Role.users).filter(User.id == current_user.id).all()
        )
