...
class AIRequest(Base):
    ...
    brokerage_id = Column(
        Integer, ForeignKey("brokerages.id"), nullable=False, index=True
    )
    request_type = Column(
        String(50), nullable=False, index=True
    )  # 'cma', 'presentation', 'marketing', 'compliance', 'general'
    request_content = Column(Text, nullable=False)
    request_metadata_json = Column(JSON, default=dict)
    status = Column(
        String(20), default="pending", index=True
    )  # 'pending', 'processing', 'ai_complete', 'human_review', 'completed', 'failed'
    ...
    )  # 'text', 'pdf', 'presentation', 'email', 'social'
    ...
    content_deliverables = relationship(
        "ContentDeliverable", back_populates="request", cascade="all, delete-orphan"
    )
    processing_analytics = relationship(
        "AIProcessingAnalytic", back_populates="request", cascade="all, delete-orphan"
    )
    ...
    def request_metadata_dict(self):
        ...
        try:
            return (
                json.loads(self.request_metadata_json)
                if isinstance(self.request_metadata_json, str)
                else self.request_metadata_json
            )
...
class HumanExpert(Base):
    ...
    expertise_area = Column(
        String(100), nullable=False, index=True
    )  # 'market_analysis', 'presentations', 'compliance', 'marketing', 'general'
    ...
    def working_hours_dict(self):
        ...
        try:
            return (
                json.loads(self.working_hours)
                if isinstance(self.working_hours, str)
                else self.working_hours
            )
        except (json.JSONDecodeError, TypeError):
...
class ContentDeliverable(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    request_id = Column(
        Integer, ForeignKey("ai_requests.id"), nullable=False, index=True
    )
    content_type = Column(
        String(50), nullable=False, index=True
    )  # 'cma', 'presentation', 'email', 'social_post', 'document'
...
class VoiceRequest(Base):
    ...
    processing_status = Column(
        String(20), default="pending", index=True
    )  # 'pending', 'transcribing', 'processed', 'failed'
...
class TaskAutomation(Base):
    ...
    task_type = Column(
        String(50), nullable=False, index=True
    )  # 'follow_up', 'report_generation', 'client_communication', 'data_entry'
    ...
    def trigger_conditions_dict(self):
        ...
        try:
            return (
                json.loads(self.trigger_conditions_json)
                if isinstance(self.trigger_conditions_json, str)
                else self.trigger_conditions_json
            )
        except (json.JSONDecodeError, TypeError):
            return {}
    ...
    def execution_schedule_dict(self):
        ...
        try:
            return (
                json.loads(self.execution_schedule_json)
                if isinstance(self.execution_schedule_json, str)
                else self.execution_schedule_json
            )
        except (json.JSONDecodeError, TypeError):
            return {}
...
class SmartNurturingSequence(Base):
    ...
    sequence_type = Column(
        String(50), nullable=False, index=True
    )  # 'lead_nurturing', 'client_retention', 'follow_up', 'marketing'
    ...
    def target_audience_dict(self):
        ...
        try:
            return (
                json.loads(self.target_audience)
    ...
    def performance_metrics_dict(self):
        ...
        try:
            return (
                json.loads(self.performance_metrics)
                if isinstance(self.performance_metrics, str)
...
class DubaiPropertyData(Base):
    ...
    def market_data_dict(self):
        ...
        try:
            return (
                json.loads(self.market_data)
                if isinstance(self.market_data, str)
    ...
    def price_history_list(self):
        ...
        try:
            return (
                json.loads(self.price_history)
                if isinstance(self.price_history, str)
    ...
    def neighborhood_data_dict(self):
        ...
        try:
            return (
                json.loads(self.neighborhood_data)
                if isinstance(self.neighborhood_data, str)
                else self.neighborhood_data
            )
        except (json.JSONDecodeError, TypeError):
            return {}
...
class RERAComplianceData(Base):
    ...
    compliance_type = Column(
        String(50), index=True
    )  # 'listing', 'transaction', 'disclosure', 'documentation'
    ...
    def required_documents_list(self):
        ...
        try:
            return (
                json.loads(self.required_documents)
                if isinstance(self.required_documents, str)
                else self.required_documents
            )
        except (json.JSONDecodeError, TypeError):
            return []
    ...
    def submitted_documents_list(self):
        ...
        try:
            return (
                json.loads(self.submitted_documents)
                if isinstance(self.submitted_documents, str)
                else self.submitted_documents
            )
        except (json.JSONDecodeError, TypeError):
            return []
    ...
    def regulatory_updates_list(self):
        ...
        try:
            return (
                json.loads(self.regulatory_updates)
                if isinstance(self.regulatory_updates, str)
                else self.regulatory_updates
            )
        except (json.JSONDecodeError, TypeError):
            return []
...
class RetentionAnalytic(Base):
    ...
    metric_name = Column(
        String(100), nullable=False, index=True
    )  # 'lead_conversion', 'client_retention', 'follow_up_success'
