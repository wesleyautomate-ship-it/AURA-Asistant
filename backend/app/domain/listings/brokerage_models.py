...
class BrandAsset(Base):
    ...
    )  # 'logo', 'template', 'color_scheme', 'font'
...
class LeadRetentionAnalytic(Base):
    ...
    )  # Will reference leads table when it exists
...
class WorkflowEfficiencyMetric(Base):
    ...
    workflow_id = Column(
        Integer, ForeignKey("workflow_automation.id"), nullable=False, index=True
    )
