...
class EnhancedLead(Base):
    ...
    def preferred_areas_list(self):
        ...
        try:
            return (
                json.loads(self.preferred_areas)
                if isinstance(self.preferred_areas, str)
                else self.preferred_areas
            )
        except (json.JSONDecodeError, TypeError):
            return []
    ...
    def source_details_dict(self):
        ...
        try:
            return (
                json.loads(self.source_details)
                if isinstance(self.source_details, str)
                else self.source_details
            )
        except (json.JSONDecodeError, TypeError):
            return {}
...
class Transaction(Base):
    ...
    buyer = relationship(
        "EnhancedClient", foreign_keys=[buyer_id], back_populates="transactions"
    )
...
__table_args__ = (
    ...
    CheckConstraint(
        "listing_status IN ('draft', 'live', 'sold', 'withdrawn', 'pending')",
        name="chk_properties_listing_status",
    ),
    ...
    CheckConstraint(
        "nurture_status IN ('new', 'hot', 'warm', 'cold', 'qualified', 'unqualified')",
        name="chk_leads_nurture_status",
    ),
    CheckConstraint(
        "lead_score >= 0 AND lead_score <= 100", name="chk_leads_lead_score"
    ),
    CheckConstraint(
        "urgency_level IN ('low', 'normal', 'high', 'urgent')",
        name="chk_leads_urgency_level",
    ),
    CheckConstraint(
        "client_type IN ('buyer', 'seller', 'investor', 'tenant', 'landlord')",
        name="chk_clients_client_type",
    ),
    CheckConstraint(
        "client_status IN ('active', 'inactive', 'prospect', 'closed')",
        name="chk_clients_client_status",
    ),
    CheckConstraint(
        "transaction_type IN ('sale', 'rental', 'lease', 'investment')",
        name="chk_transactions_type",
    ),
    CheckConstraint(
        "transaction_status IN ('pending', 'in_progress', 'completed', 'cancelled', 'on_hold')",
