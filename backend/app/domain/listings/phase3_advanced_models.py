...
class PredictivePerformanceModel(Base):
    ...
    )  # 'agent_performance', 'lead_conversion', 'market_trend', 'client_retention'
    ...
    def feature_importance_dict(self):
        ...
        try:
            return (
                json.loads(self.feature_importance)
...
class BenchmarkingData(Base):
    ...
    benchmark_type = Column(
        String(50), nullable=False, index=True
    )  # 'agent_performance', 'lead_conversion', 'client_retention', 'market_share'
...
class DubaiMarketData(Base):
    ...
    data_type = Column(
        String(50), nullable=False, index=True
    )  # 'price_per_sqft', 'rental_yield', 'occupancy_rate', 'transaction_volume'
    ...
    )  # 'RERA', 'Dubai Land Department', 'Property Finder', 'Bayut'
...
class RERAIntegrationData(Base):
    ...
    completion_status = Column(
        String(50)
    )  # 'completed', 'under_construction', 'planned'
    ...
    compliance_status = Column(
        String(20), default="compliant", index=True
    ...
    def payment_plan_dict(self):
        ...
        try:
            return (
                json.loads(self.payment_plan)
                if isinstance(self.payment_plan, str)
                else self.payment_plan
            )
        except (json.JSONDecodeError, TypeError):
            return {}
    ...
    def nearby_facilities_list(self):
        ...
        try:
            return (
                json.loads(self.nearby_facilities)
                if isinstance(self.nearby_facilities, str)
                else self.nearby_facilities
            )
        except (json.JSONDecodeError, TypeError):
            return []
    ...
    def transportation_links_list(self):
        ...
        try:
            return (
                json.loads(self.transportation_links)
                if isinstance(self.transportation_links, str)
                else self.transportation_links
            )
        except (json.JSONDecodeError, TypeError):
            return []
...
class SystemPerformanceMetric(Base):
    ...
    metric_category = Column(
        String(50), nullable=False, index=True
    )  # 'api_performance', 'database_performance', 'ai_processing', 'user_activity'
    metric_name = Column(String(100), nullable=False)
    metric_value = Column(DECIMAL(15, 4), nullable=False)
    metric_unit = Column(
        String(20)
    )  # 'milliseconds', 'requests_per_second', 'percentage', 'count'
...
class UserActivityAnalytic(Base):
    ...
    activity_type = Column(
        String(50), nullable=False, index=True
    )  # 'login', 'request_creation', 'content_download', 'expert_review'
    ...
    def activity_details_dict(self):
        ...
        try:
            return (
                json.loads(self.activity_details)
                if isinstance(self.activity_details, str)
                else self.activity_details
            )
        except (json.JSONDecodeError, TypeError):
            return {}
...
class AIProcessingAnalytic(Base):
    ...
    processing_stage = Column(
        String(50), nullable=False, index=True
    )  # 'request_analysis', 'ai_generation', 'human_review', 'content_delivery'
...
class MultiBrokerageAnalytic(Base):
    ...
    analytics_type = Column(
        String(50), nullable=False, index=True
    )  # 'system_usage', 'feature_adoption', 'performance_comparison', 'market_insights'
    metric_name = Column(String(100), nullable=False)
    metric_value = Column(DECIMAL(15, 4), nullable=False)
    ...
    def additional_metrics_dict(self):
        ...
        try:
            return (
                json.loads(self.additional_metrics)
                if isinstance(self.additional_metrics, str)
                else self.additional_metrics
            )
        except (json.JSONDecodeError, TypeError):
            return {}
...
class DeveloperPanelSetting(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False, index=True)
    setting_category = Column(
        String(50), nullable=False, index=True
    )  # 'monitoring', 'alerts', 'analytics', 'system_control'
...
class SystemAlert(Base):
    ...
    alert_type = Column(
        String(50), nullable=False, index=True
    )  # 'error', 'warning', 'info', 'performance'
    alert_category = Column(
        String(50), nullable=False, index=True
    )  # 'system', 'database', 'ai_processing', 'user_activity'
    ...
    def affected_components_list(self):
        ...
        try:
            return (
                json.loads(self.affected_components)
                if isinstance(self.affected_components, str)
                else self.affected_components
            )
        except (json.JSONDecodeError, TypeError):
            return []
    ...
    def alert_data_dict(self):
        ...
        try:
            return (
                json.loads(self.alert_data)
                if isinstance(self.alert_data, str)
                else self.alert_data
            )
        except (json.JSONDecodeError, TypeError):
            return {}
